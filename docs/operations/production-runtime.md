...
Company-logo storage is configured through `COMPANY_ASSETS_DISK` and defaults to `company_assets_local`, whose root is `storage/app/company-assets`. This disk is private, has no public symbolic link or temporary-serving route, and requires the PHP `fileinfo` extension for content detection. Production upload validation must use bounded container/metadata inspection and must not fully decode untrusted raster pixels into GD memory. GD remains a development/test dependency for generated fixtures, not a production upload-validation dependency. Stored-file SHA-256 verification is streamed so it does not retain a second complete copy. The production-readiness command rejects a missing, public, or framework-served Company-asset disk.
