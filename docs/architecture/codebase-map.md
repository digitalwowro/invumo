...
A recurring template remains a separate aggregate with inheritance intent, not a third `DocumentKind`, and must not be forced through Document-model lifecycle code. Phase 10 reuses the shared React editor, decimal calculations, line input contracts, selection/default resolution, and snapshot semantics. Occurrence generation creates a real Invoice through an Invoices-owned transaction-neutral generation boundary built on the same `Documents` numbering/snapshot/line primitives; automatic email composes the existing `Delivery` workflow. It must not copy Quote/Invoice Draft orchestration or delivery composition into `Recurring`.
