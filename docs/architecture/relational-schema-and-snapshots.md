...
The Owner-only, recently reauthenticated workflow requires an exact workspace-name entry, a separate permanent-action acknowledgement, and an opaque state version. Under the Company context it locks delivery attempts, artifacts, dispatcher rows, and Company assets in stable order; cancels unclaimed domain dispatches; and blocks while a provider submission has an unknown in-flight outcome. The same transaction inserts a privacy-minimal non-tenant erasure proof, copies each unique private storage coordinate plus a non-secret location fingerprint into `company_erasure_files`, and only then deletes the Company so tenant rows and memberships follow their approved owned-child cascades.

The encrypted after-commit job carries only the erasure-event UUID. It reads the durable manifest, attempts every pending file independently, confirms absence, and atomically clears the disk/key/fingerprint only on completion. One unreadable file does not prevent later files from being attempted. Bounded job retries record a non-disclosing category; a scheduled reconciliation entry point requeues manifest rows left pending after lost dispatch, exhausted retry, or failed-job maintenance. A changed local root or object-storage location fails closed against the retained fingerprint. Runtime may never delete a manifest row or reopen completed evidence. Platform Operations exposes the privacy-minimal event and only aggregate complete/pending/attention counts, never storage coordinates. A Company deletion must not be implemented as an unreviewed controller-level cascade.
...
- Dependency guards and deletion-state versions prevent forbidden or stale-confirmation deletes, while approved document, User/Account, and whole-Company erasure remove live sensitive rows, retain only approved minimal proofs, and preserve retryable cleanup coordinates until private-file absence is confirmed.
