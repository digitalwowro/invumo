...
class UpdateInvoiceDraftRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'payment_term_days' => [
                ...
                'max:'.ContentLimits::MAX_CALENDAR_DAY_OFFSET,
            ],
            'due_date' => ['present', 'nullable', 'date_format:Y-m-d'],
            'customer_reference' => [
                'present', 'nullable', 'string',
                'max:'.ContentLimits::CUSTOMER_REFERENCE_CHARACTERS,
            ],
            'bank_account_id' => ['present', 'nullable', 'uuid'],
            'terms_and_conditions' => ['present', 'nullable', 'string', 'max:'.ContentLimits::TERMS_AND_CONDITIONS_CHARACTERS],
            'notes' => ['present', 'nullable', 'string', 'max:'.ContentLimits::NOTES_CHARACTERS],
            'lines' => ['present', 'array'],
    ...
    public function draft(): InvoiceDraftData
    {
        ...
        return new InvoiceDraftData(
            ...
            paymentTermDays: $this->validated('payment_term_days') === null
                ? null
                : (int) $this->validated('payment_term_days'),
            dueDate: $this->stringOrNull($this->validated('due_date')),
    ...
    protected function prepareForValidation(): void
    {
        ...
        $this->merge([
            ...
            'payment_term_days' => $this->nullableInput('payment_term_days'),
            'due_date' => $this->nullableInput('due_date'),
    ...
    public function after(): array
    {
        return [function (Validator $validator): void {
            ...
            $paymentDays = $this->input('payment_term_days');

            if (is_string($issueDate) && is_numeric($paymentDays)) {
                try {
                    DocumentCalendar::addDays($issueDate, (int) $paymentDays);
                } catch (InvalidArgumentException) {
                    $validator->errors()->add('payment_term_days', __('invoices_ui.errors.due_date_out_of_range'));
                }
            }

            if (is_string($issueDate) && is_string($dueDate) && $dueDate < $issueDate) {
                $validator->errors()->add('due_date', __('invoices_ui.errors.due_date_before_issue'));
            }
        }];
    }
    ...
    private function decimal(string $kind): Closure
    {
        return function (string $attribute, mixed $value, Closure $fail) use ($kind): void {
            ...
            try {
                ...
                $fail(__('invoices_ui.errors.decimal_invalid'));
            }
        };
    }
