...
final readonly class RestoreCompanyCurrency
{
    ...
    public function handle(Company $company, User $actor, string $currencyId): CompanyCurrency
    {
        return $this->tenantContext->runForMember(
            ...
            fn (): CompanyCurrency => DB::connection(config('database.tenant_connection'))
                ->transaction(fn (): CompanyCurrency => $this->restore(
    ...
    private function restore(
        ...
        if ($currency->active) {
            throw CompanyCurrencyException::active();
        }
        ...
        $this->recordAuditEvent->handle(new AuditEventData(
            ...
            action: 'company.currency.restored',
            targetType: 'CompanyCurrency',
            targetId: $currency->id,
            before: $this->auditPayload($currency->currency_code, false),
            after: $this->auditPayload($currency->currency_code, true),
        ));
