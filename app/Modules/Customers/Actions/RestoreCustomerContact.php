...
final readonly class RestoreCustomerContact
{
    ...
    public function handle(Company $company, User $actor, string $customerId, string $contactId): void
    {
        $this->tenantContext->runForMember(
            ...
            fn () => DB::connection(config('database.tenant_connection'))
                ->transaction(fn () => $this->restore($company, $actor, $customerId, $contactId)),
        );
    }

    private function restore(Company $company, User $actor, string $customerId, string $contactId): void
    {
        ...
        if ($contact->archived_at === null) {
            throw CustomerContactException::notArchived();
        }
        ...
        $this->recordAuditEvent->handle(new AuditEventData(
            ...
            action: 'company.customer_contact.restored',
