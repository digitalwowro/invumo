...
| ---------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| Create                 | First Save from an unsaved editor in an authorized Company context                                      | Persist the submitted `DRAFT` with an allocated number | Audit creation with bounded initial-content metadata; idempotent first Save                       |
...
| Action              | Preconditions                                                                      | Result                                              | External/history effects                                             |
| ------------------- | ---------------------------------------------------------------------------------- | --------------------------------------------------- | -------------------------------------------------------------------- |
| Create              | First Save from an unsaved editor in an authorized Company context                 | Persist the submitted `DRAFT` with allocated number | Audit creation with bounded initial-content metadata; idempotent first Save |
...
### Permission-aware cancellation escalation
...
The typed resolver implements this complete `invoice_total`/`net_paid` comparison. Phase 6 initially used its named no-financial-rows entry point, which supplied zero net paid. Batch 7A supplies the authoritative transaction-ledger aggregate to Invoice edit, list, current-screen, and PDF callers, so Unpaid, Partially Paid, Paid, and Overdue remain one derivation rather than separate UI interpretations.
...
- A previously sent payment-received email does not freeze the transaction. A later edit/delete remains possible after warning, full invariant validation, and audit; the delivered email remains an immutable document-level historical artifact bound to the Payment edit version from which its amount, date, and outstanding balance were resolved. Same-kind correction makes an unsent frozen receipt ineligible for submission/retry. Changing the row away from Payment or deleting it clears only the delivery's nullable live transaction reference, never its resolved content, version, recipients, attempts, or provider events.
...
- Quote and Invoice edit/list Queries expose the current blockers and customer-exposure warning level before confirmation. Those values are advisory UX only: the owning deletion Action locks the aggregate, delivery state, provenance, and financial dependencies and repeats every guard before mutation. A queued or retrying delivery also makes transaction controls visibly unavailable while the transaction Actions retain their independent locked recheck.
...
- When the Invoice was generated by a recurring occurrence, deletion also removes that occurrence and its pending dispatch state without rewinding the template schedule, ordinal, or successful-occurrence count. Stale occurrence work cannot recreate the deleted period; later distinct scheduled occurrences continue normally. Cancellation retains the occurrence and likewise does not stop later occurrences.
