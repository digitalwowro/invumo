...
5. When a change crosses several workflows or the consumer inventory cannot be bounded confidently, run the complete local browser suite through the supervised Composer wrapper. Dynamic CI shards are execution and database-isolation partitions whose file membership may change; never use shard membership to select impacted consumers. This is broadened rendered-UI verification, not authorization for the complete local repository gate or a GitHub phase gate outside phase closeout.
