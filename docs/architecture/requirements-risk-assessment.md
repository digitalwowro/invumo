...
- Public-launch operations verification remains a valid just-in-time gate. The route/composition, upload, PDF-renderer, public-token, ZeptoMail document-delivery/webhook, and hosted-runtime gates are complete. None of the remaining gates reopens the approved schema.
...
| Public tokens                        | Complete — Phase 8A                                       | Implement the approved [`public-token-and-access.md`](public-token-and-access.md) token, storage, expiry, regeneration, rate-limit, logging, privacy, and RLS-bootstrap contract in Batches 8B/8C                                  |
| ZeptoMail/document webhooks          | Complete — Phase 9A                                       | Document email uses the HTTPS Send API; authenticated privacy-minimal webhooks, explicit unknown outcomes, normalized ordering/idempotency, and document-bound content erasure follow the approved contract                        |
