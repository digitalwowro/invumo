...
- Next gate: Implement the Platform Operations control-plane schema, then its authorization/bootstrap boundary and guarded back-office vertical slices
...
| Platform Operations identity, visibility, manual plan lifecycle, suspension, audit, routes, and no-tenant-bypass contract                    | Complete                                                            | Implementing platform operator/control-plane schema or internal back-office routes        | Phase 1                                         |
...
- [x] Review and approve the [Platform Operations contract](../architecture/platform-operations.md), including the separate Platform Owner role, control-plane-only visibility, manual provider-independent Account plan lifecycle, User/Account suspension, append-only platform audit, `/platform` shell, and v1 exclusion of tenant-data access, impersonation, checkout, and payment collection.
...
- [ ] Implement the Platform Operations control-plane schema: Platform Owner records, User/Account suspension and last-login fields, current Account plan lifecycle/status/date fields, supporting constraints/indexes/grants, and append-only platform audit.
- [ ] Implement current-operator authorization, the protected Platform Owner grant/revoke command with last-operator safety, bounded platform Inertia props, and a separate `/platform` shell.
- [ ] Implement guarded Platform Operations Actions/Queries and localized shared-component screens for overview, Users, Accounts, Companies, plan lifecycle/expiry, suspension/reactivation, session revocation, and platform audit without tenant-data access or impersonation.
