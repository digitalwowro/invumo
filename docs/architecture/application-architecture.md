...
The internal [Platform Operations](platform-operations.md) back office is another bounded module and React/Inertia shell inside this same application. It uses the same authentication, localization, design system, database, and deployment; it does not introduce a second service, frontend, authentication system, or general tenant-data bypass. After same-request current-password validation and throttling, full-action impersonation transitions the existing server-side session to a selected non-operator effective User, preserves the original Platform Owner separately for audit/exit, blocks Platform Operations for the impersonated session, and then uses only the selected User's normal Company authorization and RLS paths.
