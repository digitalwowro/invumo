...
- Company erasure blocks on an unknown in-flight provider submission, cancels unclaimed domain dispatches, and deletes tenant rows atomically. Before that delete commits, every private storage coordinate and a non-secret storage-location fingerprint are copied into a restricted non-tenant cleanup manifest. Encrypted after-commit cleanup retries each file independently, confirms absence before clearing its coordinate, and a scheduled reconciliation pass requeues abandoned/exhausted work. A storage-location change fails closed instead of declaring a file absent from a newly configured location. The privacy-minimal erasure proof and cleanup status remain retrievable to Platform Owners; completed coordinates are irreversibly cleared.
