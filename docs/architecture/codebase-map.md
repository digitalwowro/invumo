...
`app/Integrations` contains concrete adapters such as `ZeptoMail` and `Dompdf`. Integrations translate vendor or renderer behavior into Invumo-owned contracts and data. They do not own document state, business rules, or tenant authorization. `Delivery` owns the shared `OutwardDocument` data contract, exact display formatter, kind-aware current-representation Queries, PDF contract, Blade template, outward CSS, public-link credential lifecycle, event/language Company email templates, direct-send and explicit payment-received Actions/jobs, immutable artifact/history records, and provider-neutral send contract. Template resolution uses Laravel-authored fallbacks, exact event-specific placeholder allowlists, plain-text authored content, and a side-effect-free preview Query; Company overrides are changed only through named root Actions. Its public-document boundary uses encrypted recoverable tokens with hashed lookup, a transaction-local forced-RLS bootstrap, global pre-routing request-token redaction, and thin anonymous HTML/PDF controllers. Its outbound boundary resolves content and recipients before dispatch, persists one logical delivery plus immutable per-submission attempts, enforces shared-provider recipient budgets, renders optional PDF artifacts privately, and calls the concrete ZeptoMail HTTPS adapter only after tenant transactions close. Its inbound boundary lets the ZeptoMail adapter authenticate the pinned static header and normalize a bounded form payload, then lets a Delivery Query bootstrap only the exact attempt reference before a root Action appends the privacy-minimal event under ordinary forced RLS. `Quotes` and `Invoices` own their authenticated representation routes and thin link-management controllers, while their aggregate deletion/unlink Actions call or query the Delivery-owned lifecycle boundary. React consumes the same resolved data contract through `components/domain/outward-document*.tsx`, while the Blade template consumes it for PDF output. Source-owned PDF fonts live under `resources/fonts/atkinson-hyperlegible` with pinned origins and checksums.
...
The persisted Quote and Invoice workspaces follow the same boundary. `Quotes` and `Invoices` own their root Draft mutations and editor/list Queries; `Invoices` also owns the named issue, cancel, reopen, and guarded deletion Actions plus typed lifecycle/payment/overdue derivation and the permission-aware cancellation workflow Query. `Transactions` owns Invoice financial-row CRUD, the complete exact-decimal ledger, stable aggregate locking, per-Invoice presentation, and the read-only Company operational-list Query; Invoice Actions and Queries consume its contracts to enforce zero-net-paid cancellation and derive current state without storing a balance cache. Quote-to-Invoice conversion is a `Quotes` coordinating root Action because Quote eligibility/provenance owns the workflow; it authorizes both document abilities, owns the outer transaction, and uses the transaction-neutral `Documents` snapshot copier plus number-allocation contract rather than calling another root Action.

`Documents` owns the transaction-neutral lifecycle mechanics shared by both roots: configuration-first Draft creation, common snapshot initialization and number/audit history, customer-confirmed Draft preparation, source and line reconciliation, common totals/version finalization, and document-wide deletion-resource cleanup. Quote/Invoice root Actions still own authorization, the outer transaction, subtype locks, lifecycle-specific validation, subtype effects, and the semantic audit event. Their subtype apply boundaries are transaction-neutral and fail closed unless invoked inside the active matching-Company tenant transaction; Create and Update roots may therefore reuse identical persistence and validation without exposing a second mutation root or relying on a caller comment. Delivery implements the `Documents`-owned deletion-resource contract so new document-wide delivery/public-link resources have one enumerating cleanup boundary rather than parallel Quote and Invoice lists.

The Company dashboard is a read-only `Companies` page Query that authorizes both Invoice and transaction visibility, composes bounded Invoice and Payment aggregates, and returns explicit currency groups plus a five-row recent-Invoice projection. It consumes the `Transactions`-owned authoritative ledger aggregate also used by the Invoice list, so payment-state arithmetic has one query boundary. The React route remains a thin page composition over `features/dashboard`; reusable metric-strip, money, status, table, and empty-state concepts remain in their approved shared component layers.

The full Company audit history is a read-only `Audit` Query exposed through Company settings to Owner/Admin only. It applies explicit Company scope in addition to forced RLS, bounded literal search and actor/target/Company-local-date filters, and stable timestamp/UUID cursor pagination. It transports only presentation-required event fields and the owning Actions' already-approved payloads; correlation/idempotency references and User email remain server-side. Impersonated events expose only a neutral support-access fact, never the Platform Owner identity retained for internal governance. React composes the reusable `ActivityTimeline` and audit-change dialog from `components/app` and `components/domain`, preserving one responsive activity language for later Customer, Quote, Invoice, recurring, and membership views.

A recurring template remains a separate aggregate with inheritance intent, not a third `DocumentKind`, and must not be forced through Document-model lifecycle code. Batches 10A–10E implement its Draft editor, typed inheritance/override snapshots, terminally immutable lifecycle, duplication, Company-local recurrence scheduling, dispatch claiming, configurable bounded catch-up, proactive failed-template visibility, failure/retry state, issued-Invoice generation, optional automatic delivery, and inherited-currency latch under `app/Modules/Recurring`, with thin routes/controllers, named root Actions, purpose-specific Queries, localized Inertia pages, and feature composition under `resources/js/features/recurring`. The editor reuses shared Customer/Product selectors, inline owning-module creation boundaries, document line editing, exact-decimal preview, and one `Documents`-owned line preparation Action while retaining independent recurring persistence and root transactions. Company timezone/automation-time changes recalculate active next-run instants through the existing Company-configuration transaction. Occurrence generation creates a real Invoice through an Invoices-owned transaction-neutral generation boundary built on the same `Documents` numbering/snapshot/line primitives and issues it through the existing Invoice boundary so reminder materialization cannot diverge; automatic email composes the existing `Delivery` workflow and reuses its public-link creation, content composition, quota, sender-eligibility, version, retry, attempt completion, and pre-submission checks. Delivery calls only Recurring-owned Query/Action boundaries to recheck and confirm the latch. It must not copy Quote/Invoice Draft orchestration or delivery composition into `Recurring`. An eligible generated Invoice follows ordinary Invoice deletion: its occurrence and pending dispatch state are removed without rewinding the template cursor/count, while cancellation preserves the occurrence and both leave later distinct scheduled occurrences unaffected.

Root document Actions acquire Company configuration before counters and the Document aggregate through the shared lock boundary. Shared document-kind data maps Quote/Invoice routes to their specific view/manage abilities, including inline Customer/Product creation. The React editor controls live in `components/domain/documents`, so Quote and Invoice features compose the same selector, line, default, and inline-creation behavior without importing each other's feature internals.
...
Cross-phase invariant review is also part of test ownership. A batch that adds a foreign-key/reference, reachable state, or a new entry point to an existing operation must identify every earlier component that enumerates the affected references/states or guards the operation. The implementation centralizes the invariant where practical and adds focused integration coverage for every affected consumer/entry point. The phase-closeout review compares the complete phase diff with the prior phase release SHA and records this seam evidence in the canonical tracker before the manual quality gate can certify the phase.
