...
Clicking New Quote or New Invoice opens the complete unsaved editor without inserting a Document or consuming a number. The first successful Save validates and persists the complete Draft while allocating its number in the same PostgreSQL transaction. The allocator locks the relevant counter row with `SELECT ... FOR UPDATE`, finds the next unoccupied automatic candidate, inserts the Draft aggregate, advances the counter, and commits. An opaque idempotent creation key generated when the editor opens makes a retried first Save return the same Draft. Abandoning the editor consumes no number, and the browser never previews or reserves one.
