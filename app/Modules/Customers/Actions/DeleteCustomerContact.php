...
final readonly class DeleteCustomerContact
{
    ...
    public function handle(Company $company, User $actor, string $customerId, string $contactId): void
    {
        $this->tenantContext->runForMember(
            ...
            fn () => DB::connection(config('database.tenant_connection'))
                ->transaction(fn () => $this->delete($company, $actor, $customerId, $contactId)),
        );
    }

    private function delete(Company $company, User $actor, string $customerId, string $contactId): void
    {
        ...
        $this->recordAuditEvent->handle(new AuditEventData(
            ...
            action: 'company.customer_contact.deleted',
